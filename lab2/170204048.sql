...
--------------------------------Find frequency range of student in each course based on student count-----------------------------------------
/* Parameters :        Student count             Frequency level
                         <5                         Lowest
						 5-10                       Moderate
						 >10                        Highest
*/

select c.cname,COUNT(e.snum) As TotalStudentsEnrolled
from course c join enrolled e on c.cnum = e.cnum
GROUP BY c.cname;
...
DECLARE
    x COURSE.cname%TYPE := '&enter_course_name';
    num int;

BEGIN
    select COUNT(e.snum) INTO num from course c join enrolled e on c.cnum = e.cnum WHERE c.cname = x;
	DBMS_OUTPUT.PUT_LINE('No of enrolled students:');
	DBMS_OUTPUT.PUT_LINE(num);
	DBMS_OUTPUT.PUT_LINE('Frequency:');
	IF (num>0 AND num<5) THEN
	    DBMS_OUTPUT.PUT_LINE('Lowest');
	ELSIF (num>4 AND num<11) THEN
