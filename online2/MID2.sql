...
CREATE OR REPLACE PROCEDURE searchProcedure(val IN Emp.eid%TYPE,send OUT number) 
IS
    id Emp.eid%TYPE;
BEGIN
...
CREATE OR REPLACE FUNCTION BudgetFunction(ch IN number,A IN Emp.eid%TYPE)
RETURN NUMBER 
IS
	prnt Dept.budget%TYPE;
	send1 number;
	minm Dept.budget%TYPE;
BEGIN
    send1 := 0;
	IF ch = 1 THEN
	    minm := 9999;
	    FOR R IN(SELECT * from Works w join Dept d on w.did = d.did where w.eid = A)LOOP
	       prnt := R.budget;
	        IF prnt<minm THEN
	            minm := prnt;
	        END IF;
...
DECLARE
	...
	unnecessary number;
BEGIN
    searchProcedure(X,got);
	--DBMS_OUTPUT.PUT_LINE(got);
	unnecessary := BudgetFunction(got,X);
	DBMS_OUTPUT.PUT_LINE(unnecessary);
END;
/
